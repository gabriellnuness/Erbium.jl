...
dIdz(I,Γ,β_abs,n2,β_emis,fiber_loss,Rho) = I * (-Γ*β_abs*(1-n2) + Γ*β_emis*n2 - fiber_loss) + Rho*n2
dIpdz(I,Γ,β_abs_pump,n2,β_emis_pump,fiber_loss) = I * (-Γ*β_abs_pump*(1-n2) + Γ*β_emis_pump*n2 - fiber_loss)
...
for k = 1:iter_intervals
    ...
    for j=1:z_intervals          
            for i=1:λ_intervals+1
                ...
                fp1 = dIdz(I_forward[i,j],Γ,β_abs[i],n2[j],β_emis[i],fiber_loss,Rho)*dz
                fp2 = dIdz(I_forward[i,j]+fp1/2,Γ,β_abs[i],n2[j],β_emis[i],fiber_loss,Rho)*dz
                fp3 = dIdz(I_forward[i,j]+fp2/2,Γ,β_abs[i],n2[j],β_emis[i],fiber_loss,Rho)*dz
                fp4 = dIdz(I_forward[i,j]+fp3,Γ,β_abs[i],n2[j],β_emis[i],fiber_loss,Rho)*dz
