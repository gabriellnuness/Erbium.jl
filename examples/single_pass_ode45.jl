...
figure(); plot(λ,I_pump_forward);plot(λ,I_forward)



# solving pump_forward with 5th order 
# the differential equation should be defined as f(du,u,t,p)
# where du is derivative of u, 
# u is the variable to solve,
# t is the time
# p are the parameters
function dI_dz(I,p,t)
    β_abs_pump,β_emis_pump,n2,Γ,fiber_loss = p
...
λi = 35
n2 = 0.0
I₀ = I_pump_forward[λi,1]
p = [β_abs_pump[λi],β_emis_pump[λi],n2,Γ,fiber_loss]

prob = ODEProblem.(dI_dz,I₀,zspan,p)
sol = solve.(prob, Tsit5(),save_everystep=false, save_start=false)


# trying and solve for all λ
I_pump_z_end = zeros(size(λ_pump,1),1)
zspan = (0.0, 10.0)
@benchmark @inbounds for λi in eachindex(λ_pump)
    ...
    prob = ODEProblem(dI_dz,I₀,zspan,p)
    sol = solve(prob, Tsit5(),save_everystep=false, save_start=false)
    I_pump_z_end[λi] = only(sol.u) # saving just the last value
end
